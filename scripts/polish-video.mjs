#!/usr/bin/env node
// polish-video.mjs — turn a raw recording (a VHS terminal .mp4 here; the same
// script also handles Playwright .webm) into a polished .mp4 with a soft,
// "Screen Studio / OpenAI"–style gradient background. Shared verbatim with the
// web client's scripts/polish-video.mjs so CLI and web demos look identical.
//
// VHS records a clean terminal capture at the tape's Width x Height. This does
// the polish in pure ffmpeg, all config-driven so the treatment lives in
